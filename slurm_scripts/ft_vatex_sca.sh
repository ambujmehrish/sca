...
#SBATCH --mem=240G
#SBATCH --job-name=ft_vatex_sca
#SBATCH -o ./slurm_scripts/logs/ft_vatex_sca_%j.out
#SBATCH -e ./slurm_scripts/logs/ft_vatex_sca_%j.out
# SCA finetune on VATEX (P4 grid clone of ft_vatex.sh): GRAM recipe, model=sca + LoRA.
...
mkdir -p slurm_scripts/logs workdir/sca_ft_vatex
...
RESUME=""; ls workdir/sca_ft_vatex/ckpt/optimizer_step_*.pt >/dev/null 2>&1 && RESUME="--resume true"
echo "START $(date +%T)  SCA finetune VATEX  (init=$INIT)"
