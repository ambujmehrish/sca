...
% ---------------------------------------------------------------------
% PATCH 1  [PASTE]   main.tex, Section 4 (experimental protocol), in the
%                    paragraph that introduces the missing-modality
%                    masking. Add as the last sentence of that paragraph.
%
...
% GRAM's Gram matrix as a zero vector, which puts a zero row AND column
% into G, forces det(G)=0 for every masked clip, and reduces the ranking
% among masked clips to tie-breaking. The code does not do this
% (utils/volume.py:266-267 builds the phantom-identity Gramian), but the
% paper never said so concretely enough to prevent the assumption. This
% sentence closes it at the point of first contact.
% ---------------------------------------------------------------------

A masked modality is dropped from that clip's aggregate, never imputed
and never zero-filled; for the Gramian baseline this means the absent
axis is made orthonormal, so the determinant reduces exactly to the
sub-Gramian over the modalities the clip retains. This is a strictly
more capable treatment than its released implementation supports, which
cannot represent an incomplete clip and discards it
(Sec.~\ref{sec:maskedvol}).


% ---------------------------------------------------------------------
% PATCH 2  [DONE, verify after regenerating]   Table 3 caption.
%
% Lives in scripts/build_missing_table.py (MAIN_CAPTION), not in the
% .tex, because everything in tables_final/ must be generator output --
% scripts/check_tables_generated.py enforces it. Regenerate with
%   python3 scripts/build_missing_table.py --all
% on the cluster and the caption below is what lands in
% experiments/results/tables_final/table3_missing.tex.
%
% Reproduced here only so the wording can be reviewed without running
% the generator. DO NOT paste this into the .tex by hand.
% ---------------------------------------------------------------------
