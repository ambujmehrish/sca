...
def create_train_dataloaders(args):
    ...
    if args.data_cfg.val:
        ...
        args.run_cfg.valid_steps = 0   # no val set: skip mid-training eval; checkpoints handled by save_steps + final save
...
def build_dataloader(dataset, collate_fn, is_train, batch_size, n_workers=None, worker_init_fn=None, use_sampler=True):
    ...
    # multiprocessing start method for DataLoader workers, overridable via env GRAM_MP_CTX
    # (fork/forkserver/spawn). None => Linux default (fork). Only relevant when n_workers>0.
    mp_ctx = (os.environ.get('GRAM_MP_CTX') or None) if n_workers else None
    if use_sampler:
        if is_train:
        ...
        if is_train:
            loader = DataLoader(dataset, sampler = sampler, batch_size = batch_size,
                            ...
                            collate_fn=collate_fn, drop_last=is_train,worker_init_fn=worker_init_fn, multiprocessing_context=mp_ctx)#,shuffle=True)
        else:
            loader = DataLoader(dataset, sampler = sampler, batch_size = batch_size,
                            num_workers=n_workers, pin_memory=True,
                            collate_fn=collate_fn, drop_last=is_train,worker_init_fn=worker_init_fn, multiprocessing_context=mp_ctx)
