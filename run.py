...
def main():
    ...
    if dist.get_rank() == 0:
        ...
        _val0 = args.data_cfg.val[0] if args.data_cfg.val else {'batch_size': 0, 'vision_sample_num': 0}
        wandb.init(
        # compute nodes have no internet -> force offline regardless of WANDB_MODE env var
        mode="offline",
        ...
        "batch-size-val": _val0['batch_size'],
        "ngpus":4,
        "architecture": "GRAM",
        "dataset": args.data_cfg.train[0]['name'],
        "epochs": args.data_cfg.train[0]['epoch'],
        "name": args.run_cfg.mode+"_finetuning_" + args.data_cfg.train[0]['name'] + "_valFrame=" + str(_val0['vision_sample_num']),
        "val_frame": str(_val0['vision_sample_num']),
