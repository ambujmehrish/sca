...
MAIN_CAPTION = (
    ...
    "released checkpoint. Its release cannot represent a missing modality at all and "
    "discards such clips, so we extend it rather than penalise it: a missing modality is "
    "\\emph{not} zero-filled --- which would place a zero row and column in the Gram matrix "
    "and force $\\det G {=} 0$ for \\emph{every} masked clip, collapsing the ranking --- but "
    "is made an orthonormal phantom axis contributing a factor of exactly $1$, so the score "
    "reduces to that clip's volume over the modalities it retains "
    "(Sec.~\\ref{sec:maskedvol}). Under masking the cross-encoder reranking stage collapses every method onto its "
