...
from utils.volume import volume_computation4,volume_computation3, volume_computation5, volume_computation_masked
import wandb
...
def evaluate_ret(model, tasks, val_loader, global_step, dataset_name=None):
    ...
    ids_caption = [j for i in all_gather_list(ids_caption) for j in i]   # gather too (vgg_ret classification uses per-clip class)
    ...
    if dataset_name=='vgg_ret':
        ...
        ids_txt = class_ids          # dedup ids_txt to the unique classes too (match feat_t; fixes shape assert)


    # mode-set = which modalities THIS eval scores (unchanged); per-clip masking added underneath.
    if len(feat_s)>0:
        _feats = [feat_v,feat_a,feat_s,feat_d] if len(feat_d)>0 else [feat_v,feat_a,feat_s]
    else:
        _feats = [feat_v,feat_a]
    ...
    if dataset_name == 'vgg_ret':
        # classification: area is (num_class, num_clip); smaller volume = better match.
        # true class of clip j = class_ids.index(ids_caption[j]).  Acc@k = truth among k smallest-vol classes.
        gt = torch.tensor([class_ids.index(c) for c in ids_caption], device=area.device)
        gt_vol = area[gt, torch.arange(area.shape[1], device=area.device)]
        rank = (area < gt_vol.unsqueeze(0)).sum(0)          # #classes strictly better than the truth
        a1  = round((rank < 1).float().mean().item() * 100, 1)
        a10 = round((rank < 10).float().mean().item() * 100, 1)
    ...
    if dataset_name == 'vgg_ret':
        return val_log      # classification (Acc via cls_acc) done; skip all retrieval-only metrics below
