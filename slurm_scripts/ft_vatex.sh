...
#SBATCH -o /leonardo_work/IscrC_GMEG/anag0000/HyperAlign/slurm_scripts/logs/ft_vatex_%j.out
#SBATCH -e /leonardo_work/IscrC_GMEG/anag0000/HyperAlign/slurm_scripts/logs/ft_vatex_%j.out
...
cd "$H"; mkdir -p slurm_scripts/logs workdir/finetune_vatex
INIT="$H/workdir_v2full/4model/ckpt/best_ret%tvas--msrvtt_ret_ret_area_forward.pt"
RESUME=""; ls workdir/finetune_vatex/ckpt/optimizer_step_*.pt >/dev/null 2>&1 && RESUME="--resume true"
...
srun python3 -m torch.distributed.launch --nnodes 1 --node_rank 0 --nproc_per_node 4 --master_port 9898 \
  ...
  --output_dir ./workdir/finetune_vatex --checkpoint "$INIT" --save_best true --checkpointing true $RESUME 2>&1
echo "DONE $(date +%T)"
